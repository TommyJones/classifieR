...
RfEffectSize <- function(forest, training_data, varlist = NULL, ...){
  ...
  ExecuteContinuous <- function(variable, training_data, varlist, forest, cat_classes){
    ...
    if(nrow(d1) < 5 | nrow(d2) < 5){
      warning(paste("variable", variable, "has little or no variability. Cannot compute marginal effects."))

      result <- list(data.frame(level=paste("variable", variable,
    ...
    if(forest$type == "classification"){
      if(length(unique(d1$y)) == 1 | length(unique(d2$y)) == 1){
        warning(paste("'y' has little or no variability. Cannot compute marginal effects for variable",
                      variable, "at level", level_name))

        result <- list(data.frame(level=paste("'y' has little or no variability. Cannot compute marginal effects for variable",
                                              variable, "at level", level_name),
                                  effect_size=NA,
                                  p_values=NA,
                                  conf_95_low=NA,
  ...
  ExecuteBinary <- function(variable, training_data, varlist, forest, cat_classes){
    ...
    if(nrow(d1) < 5 | nrow(d2) < 5){
      ...
      y_lvls <- as.character(sort(unique(forest$y)))

      result <- lapply(y_lvls, function(k){
        data.frame(level=paste("Cannot compute marginal effects for variable",
                               variable, "at level", var_levels[ 1 ]),
                   effect_size=NA,
                   p_values=NA,
                   conf_95_low=NA,
                   conf_95_high=NA,
    ...
    if(forest$type == "classification"){
      if(length(unique(d1$y)) == 1 | length(unique(d2$y)) == 1){
        ...
        result <- lapply(y_lvls, function(k){
          data.frame(level=paste("'y' has little or no variability. Cannot compute marginal effects for variable",
                                 variable),
                     effect_size=NA,
                     p_values=NA,
                     conf_95_low=NA,
